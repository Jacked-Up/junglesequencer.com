...
Jungle comes with a system that helps you create and manage events to be called when the tree stops playing.

Nodes may perform some action that you want to have reverted when the tree stops playing.
Luckily, Jungle playbacks allow you to easily create and manage revert actions to solve this exact issue.

#### Example Usage

```csharp
public class SetPlayerPosition : IONode<Player>
{
    [SerializeField]
    ...
    protected override void OnStart(Player player)
    {
        ...
        AddRevertAction(RevertPlayerPosition);
    }

    private void RevertPlayerPosition()
    {
...
Let's say you have node that sets a players position during a cutscene, and when the cutscene ends, you want the player to return to the original position.
