...
The node graph is a section of the interface where you put your sequences together.
...
2. [Connection Popup](#via-the-connection-lookup)
...
![Adding nodes via the node explorer](img/node-graph-adding-nodes-via-node-explorer.gif)
...
![Adding nodes via the search menu](img/node-graph-adding-nodes-via-search-menu.gif)
...
![Selecting nodes with the cursor](img/node-graph-selecting-with-cursor.gif)
...
![Draw a box selection](img/node-graph-box-selection.gif)
...
![Connecting nodes in the graph](img/node-graph-connecting-nodes.gif)
